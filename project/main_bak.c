...
void save_quadtree(FILE *file, QuadtreeNode *node) {
    if (!node) return;
    ...
    if (node->children[0] == NULL) {
        ...
        fputc(0, file);
        for (int i = 0; i < 4; i++) {
            save_quadtree(file, node->children[i]);
        }
    }
}
...
void save_image_quadtree(const char *filename, QuadtreeNode *quadtree) {
    ...
    save_quadtree(file, quadtree);
    fclose(file);
}

QuadtreeNode* load_quadtree(FILE *file, int size, int x, int y) {
    int type = fgetc(file);
    ...
    if (type == 1) {
        ...
        node->children[0] = load_quadtree(file, half_size, x, y);
        node->children[1] = load_quadtree(file, half_size, x + half_size, y);
        node->children[2] = load_quadtree(file, half_size, x, y + half_size);
        node->children[3] = load_quadtree(file, half_size, x + half_size, y + half_size);
        return node;
    }
}
...
QuadtreeNode* load_image_quadtree(const char *filename) {
    ...
    QuadtreeNode *quadtree = load_quadtree(file, IMAGE_SIZE, 0, 0);
    fclose(file);
    return quadtree;
}

void save_quadtree_bw(FILE *file, QuadtreeNode *node) {
    if (!node) return;
    ...
    if (node->children[0] == NULL) {
        ...
        Uint8 r, g, b, a;
        MLV_convert_color_to_rgba(node->color, &r, &g, &b, &a);
        Uint8 bw = (r + g + b) / 3; // Convert to grayscale
        fwrite(&bw, 1, 1, file);
    } else {
        // Internal node
        fputc(0, file);
        for (int i = 0; i < 4; i++) {
            save_quadtree_bw(file, node->children[i]);
        }
    }
}
...
void save_image_quadtree_bw(const char *filename, QuadtreeNode *quadtree) {
    ...
    save_quadtree_bw(file, quadtree);
    fclose(file);
}
...
void draw_buttons() {
    ...
    y += button_height + padding;
    MLV_draw_text_box(x, y, button_width, button_height, "Load Image", 1, MLV_COLOR_WHITE, MLV_COLOR_BLACK, MLV_COLOR_GRAY, MLV_TEXT_CENTER, MLV_HORIZONTAL_CENTER, MLV_VERTICAL_CENTER);
...
int handle_button_click(int x, int y) {
    ...
    by += button_height + padding;
    if (x > bx && x < bx + button_width && y > by && y < by + button_height) return 5; // Load Image
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        switch (button) {
            ...
            case 3:
                if (quadtree) {
                    save_image_quadtree_bw("quadtree.qtn", quadtree);
                }
                break;
            case 4:
                if (quadtree) {
                    save_image_quadtree("quadtree.qtc", quadtree);
                }
                break;
            case 5:
                quadtree = load_image_quadtree("quadtree.qtc");
                if (quadtree) {
